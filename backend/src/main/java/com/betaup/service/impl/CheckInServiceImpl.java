...
public class CheckInServiceImpl implements CheckInService {
    ...
    public ApiResponse<CheckInResult> checkIn(Long userId, CheckInRequest request) {
        ...
        if (checkInRepository.existsByUserIdAndGymIdAndCheckDate(userId, gym.getId(), today)) {
            return ApiResponse.success("Already checked in at this gym today.", buildResult(null, gym, false, List.of()));
        }
        ...
        // Evaluate badges and collect newly unlocked ones
        List<BadgeProgressDto> progressBefore = badgeAutomationService.getProgressForUser(user);
        badgeAutomationService.evaluateUserBadges(user);
        List<BadgeProgressDto> progressAfter = badgeAutomationService.getProgressForUser(user);

        List<String> newBadgeKeys = progressAfter.stream()
            .filter(BadgeProgressDto::isEarned)
            .filter(after -> progressBefore.stream()
                .anyMatch(before -> before.getBadgeKey().equals(after.getBadgeKey()) && !before.isEarned()))
            .map(BadgeProgressDto::getBadgeKey)
            .toList();

        return ApiResponse.success(gpsVerified ? "GPS check-in successful!" : "Manual check-in recorded.",
            buildResult(saved, gym, gpsVerified, newBadgeKeys));
    }

    private CheckInResult buildResult(CheckIn checkIn, Gym gym, boolean gpsVerified, List<String> newBadgeKeys) {
